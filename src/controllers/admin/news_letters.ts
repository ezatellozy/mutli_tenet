...
import NewsLetters from "../../models/news_letters"
...
export const newsIndex = async (_req: Request, res: Response): Promise<void> => {
    try {
        const allContacts = await NewsLetters.findAll({
            attributes: ["id", "email"]
        });
        // const response = allSettings.reduce((acc, setting) => {
        //     acc[setting.key] = setting.value;
